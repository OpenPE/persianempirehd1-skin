...
class pliSensors(Converter, object):
    RPM = 0
    RPMfull = 1
    RPM2000 = 2
    RPM3000 = 3
    ...
    def getText(self):
        ...
        if self.type == self.RPMTEMPMAX:
            ...
            if self.type == self.RPMfull:
                rpm = self.getFanRPM()
                return self.w2 + str(rpm) + ' rpm'
            if self.type == self.TEMPMAX:
                tmp = self.getTempMax()
                return self.w2 + str(tmp)
            if self.type == self.TEMPMAXfull:
                tmp = self.getTempMax()
                return self.w2 + str(tmp) + '\xc2\xb0C'
            return ''
